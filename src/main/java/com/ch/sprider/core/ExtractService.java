...
public class ExtractService {
	 ...
	 * @return
	 */
	public static List<?> extract(Rule rule,String traderId,String traderName) {
		// 进行对rule的必要校验
		validateRule(rule);
		List<TradeData> datas = new ArrayList<TradeData>();
		...
		try {
			...
			// 解析获取的数据
			datas = praseHtml4JRQ(results,traderId,traderName);
	 ...
	 * 普通网页的数据解析
	 */
	private static List<LinkTypeData> praseResule4Net(Elements results,
	 ...
	 * 解析金融圈页面数据
	 */
	private static List<TradeData> praseHtml4JRQ(Elements results,String traderId,String traderName) {
		List<TradeData> tradeList = new ArrayList<TradeData>();
		for (Element result : results) {
			String data = result.select("div[data-order]").attr("data-order");
			TradeData tradeData = new TradeData(data,traderId,traderName);
